...
export const errorHandler = (error, config = {}) => {
  ...
  if (response && response.status) {
    ...
    const jumped = jumpLogin(response);
    const { status, url } = response;
    showErrorMessage &&
      showError({
        message: errorMessage || `请求错误 ${status}: ${url}`,
        description: errorText,
      });
  } else if (isSuccess === false && showErrorMessage) {
    showError({ message: errorMessage });
  } else if (!response && showErrorMessage) {
    showError({
      description: "您的网络发生异常，无法连接服务器",
      message: errorMessage || "网络异常",
    });
  }
  return error;
  // throw error;
